...
function compare(
  a: string | any[] | Uint8Array | Uint16Array | Uint32Array,
...
export class AssetTransfer extends Contract {
  ...
  public async ReadAsset(ctx: Context, assetID: string): Promise<RealEstate> {
    console.log(
      `ReadAsset: collection ${AssetTransfer.ASSET_COLLECTION_NAME}, ID ${assetID}\n`
    );
    let assetJSON: Uint8Array = await ctx.stub.getPrivateData(
      AssetTransfer.ASSET_COLLECTION_NAME,
      assetID
    );
  ...
  public async ReadAssetPrivateDetails(
    ...
    collection: string,
    assetID: string
  ): Promise<AssetPrivateDetails> {
    console.log(
      `ReadAssetPrivateDetails: collection %${collection}, ID ${assetID}\n`
    );
    let assetPrvJSON: Uint8Array = await ctx.stub.getPrivateData(
      collection,
      assetID
    );
    ...
    if (assetPrvJSON == null || assetPrvJSON.length == 0) {
      console.log(
        `AssetPrivateDetails ${assetID} does not exist in collection ${collection}`
      );
      return null;
    }

    let assetpd: AssetPrivateDetails =
      AssetPrivateDetails.deserialize(assetPrvJSON);
    return assetpd;
  }
  ...
  public async ReadTransferAgreement(
    ...
    let aggKey: string = ctx.stub.createCompositeKey(
      AssetTransfer.AGREEMENT_KEYPREFIX,
      [assetID]
    );
    console.log(
      `ReadTransferAgreement Get: collection ${AssetTransfer.ASSET_COLLECTION_NAME}, ID ${assetID}, Key ${aggKey}\n`
    );
    let buyerIdentity: Uint8Array = await ctx.stub.getPrivateData(
      AssetTransfer.ASSET_COLLECTION_NAME,
      aggKey.toString()
    );
    ...
    if (buyerIdentity == null || buyerIdentity.length == 0) {
      console.log(
        `BuyerIdentity for asset ${assetID} does not exist in TransferAgreement`
      );
      return null;
    }
  ...
  public async CreateAsset(ctx: Context): Promise<RealEstate> {
    ...
    let asset: RealEstate = new RealEstate(
      assetID,
      area,
      location,
      owner,
    ...
    console.log(
      `CreateAsset Put: collection ${AssetTransfer.ASSET_COLLECTION_NAME}, ID ${assetID}\n`
    );
    console.log(
      `Put: collection ${
        AssetTransfer.ASSET_COLLECTION_NAME
    ...
    let assetPriv: AssetPrivateDetails = new AssetPrivateDetails(
      assetID,
      appraisedValue
    );
    console.log(
      "Put AssetPrivateDetails: collection ${orgCollectionName}, ${assetID}\n"
    );
  ...
  async AgreetoTransfer(ctx: Context): Promise<void> {
    ...
    try {
      ...
      assetPriv = new AssetPrivateDetails(
        assetID,
        <number>json["appraisedValue"]
      );
    } catch (Error) {
    ...
    //Save AssetPrivateDetails to org collection
    console.log(
      `Put AssetPrivateDetails: collection ${orgCollectionName}, ID ${assetID}\n`
    );
    ctx.stub.putPrivateData(orgCollectionName, assetID, assetPriv.serialize());
    ...
    console.log(
      `AgreeToTransfer Put: collection ${AssetTransfer.ASSET_COLLECTION_NAME}, ID ${assetID}, Key ${aggKey}\n`
    );
    ctx.stub.putPrivateData(
      AssetTransfer.ASSET_COLLECTION_NAME,
      aggKey.toString(),
  ...
  public async TransferAsset(ctx: Context): Promise<void> {
    ...
    let transferAgreement: TransferAgreement = await this.ReadTransferAgreement(
      ctx,
      assetID
    );
    ...
    console.log(
      `Transfer Asset: collection ${AssetTransfer.ASSET_COLLECTION_NAME}, ID ${assetID} to owner ${newOwner}\n`
    );
    ctx.stub.putPrivateData(
      AssetTransfer.ASSET_COLLECTION_NAME,
      assetID,
    ...
    console.log(
      `AgreeToTransfer deleteKey: collection ${AssetTransfer.ASSET_COLLECTION_NAME}, ID ${assetID}, Key ${aggKey}\n`
    );
  ...
  public async DeleteAsset(ctx: Context): Promise<void> {
    ...
    let ownersCollectionName: string = AssetTransfer.getCollectionName(ctx);
    let apdJSON: Uint8Array = await ctx.stub.getPrivateData(
    ...
    console.log(
      `DeleteAsset: collection ${AssetTransfer.ASSET_COLLECTION_NAME}, ID ${assetID}\n`
    );
  ...
  private async verifyAgreement(
    ...
    if (!(clienID == owner)) {
      doFail(
        `Submitting client identity does not own the asset: ${AssetTransferErrors.INVALID_ACCESS.toString()}`
      );
    }
    ...
    let ownerAppraisedValueHash: Uint8Array = await ctx.stub.getPrivateDataHash(
      collectionOwner,
      assetID
    );
    ...
    let buyerAppraisedValueHash: Uint8Array = await ctx.stub.getPrivateDataHash(
      collectionBuyer,
      assetID
    );
    ...
    if (!equal(ownerAppraisedValueHash, buyerAppraisedValueHash)) {
      doFail(
        `Hash for appraised value for owner ${ownerAppraisedValueHash} does not match value for seller ${buyerAppraisedValueHash}`
      );
    }
  }
}
